        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="z-50 w-full max-w-md p-6 bg-white rounded-lg shadow-xl">
                @if($isPaymentApproved)
                    ...
                    <h3 class="mb-2 text-xl font-semibold text-center text-gray-900">পেমেন্ট অনুমোদিত হয়েছে!</h3>
                    <p class="mb-6 text-center text-gray-600">মেম্বারকে WhatsApp এ মেসেজ পাঠিয়ে জানিয়ে দিন।</p>

                    @if($selectedPaymentForApprove && $selectedPaymentForApprove->user && $selectedPaymentForApprove->user->phone)
                        @php
                            $orgName = app(\App\Services\SettingsService::class)->get('organization_name', 'প্রজন্ম উন্নয়ন মিশন');
                            $phoneForWa = preg_replace('/[^0-9]/', '', $selectedPaymentForApprove->user->phone);
                        ...
                        <div class="mb-6 text-center">
                            <a href="https://wa.me/{{ $phoneForWa }}?text={{ $message }}" target="_blank"
                               class="inline-flex items-center justify-center px-4 py-2 space-x-2 text-sm font-medium text-white transition-colors bg-green-500 rounded-lg hover:bg-green-600">
                ...
                @else
                    ...
                    <div class="flex justify-center space-x-3">
                        ...
                        <button wire:click="approvePayment"
                                wire:loading.attr="disabled"
                                class="px-6 py-2 text-white transition bg-green-600 rounded-lg hover:bg-green-700 disabled:opacity-50">
                            <span wire:loading.remove wire:target="approvePayment">নিশ্চিত করে অনুমোদন করুন</span>
                            <span wire:loading wire:target="approvePayment">অনুমোদন হচ্ছে...</span>
                        </button>
                    </div>
                @endif
            </div>
